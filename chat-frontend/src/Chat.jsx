...
const Chat = ({ onLogout }) => {
    ...
    useEffect(() => {
        if (socket && rooms.length && !currentRoom) selectRoom(rooms[0]);
        if (socket) {
    ...
    const loadRooms = async () => {
        try {
            const [roomsData, globalRoom] = await Promise.all([api.getRooms(), api.getGlobalRoom()]);
            const other = roomsData.filter((r) => !r.isGlobal);
            setRooms([globalRoom, ...other]);
        } catch (e) {
            console.error(e);
        }
    };
            ...
            <div className="sidebar">
                <div className="sidebar-header">
                    ...
                    <div className="header-actions">
                        <button onClick={() => setShowUserList(true)} className="btn-icon" title="New Chat">💬</button>
                        <button onClick={() => setShowGroupModal(true)} className="btn-icon" title="New Group">👥</button>
